  ...
  return (
    <div className="bg-app-surface border border-app-border p-4 md:p-6 rounded-2xl w-full">
      <div className="flex justify-between items-center mb-4">
        ...
        <span className="text-[11px] uppercase tracking-wider text-app-text-s font-mono">{currentMonth.format('MMMM YYYY')}</span>
      </div>
      <div className="grid grid-cols-7 gap-1.5 max-w-sm mx-auto w-full">
         ...
         {days.map(day => {
           ...
           if (status === 'ACHIEVED') bgClass = 'bg-[#c0ff00]';
           ...
           return (
             ...
             >
                <span className={`text-[10px] font-mono ${status === 'NONE' ? 'text-app-text-s' : 'text-zinc-900 font-bold'}`}>
                  {day.format('D')}
                </span>
                {isToday && <div className={`absolute bottom-0.5 w-1 h-1 rounded-full ${status === 'NONE' ? 'bg-app-text-s' : 'bg-zinc-900'}`}/>}
             </div>
           );
         })}
      </div>
